...
public class LocalBucket implements Bucket, BenchmarkEnabled {
	private class DataHelper {
		...
		public void init(File dataPath) throws IOException {
			...
			m_out = new DataOutputStream(new BufferedOutputStream(
					new FileOutputStream(m_path, true), SEGMENT_SIZE));
	...
	private class IndexHelper {
		private class Header {
			...
			private Integer findSegment(int ip, int index,
				...
				if (segmentId == null && createIfNotExists) {
					...
					if (segmentId % ENTRY_PER_SEGMENT == 0) { // last segment is
																...
																// new one
						m_segment.flush();
						m_segment = new Segment(m_channel, segmentId
								* SEGMENT_SIZE);
						m_offset = 0;
					}
			...
			public int getOffset(int ip, int seq, boolean createIfNotExists)
					throws IOException {
				int segmentIndex = seq / MESSAGE_PER_SEGMENT;
				int segmentOffset = (seq % MESSAGE_PER_SEGMENT)
						* BYTE_PER_MESSAGE;
				Integer segmentId = findSegment(ip, segmentIndex,
						createIfNotExists);

				if (segmentId != null) {
					int offset = segmentId.intValue() * SEGMENT_SIZE
							+ segmentOffset;
		...
		private class Segment {
			...
			private Segment(FileChannel channel, long address)
					throws IOException {
				m_channel = channel;
				m_address = address;
			...
			public String toString() {
				return String.format("%s[address=%s]", getClass()
						.getSimpleName(), m_address);
			}
		...
		private static final int MESSAGE_PER_SEGMENT = SEGMENT_SIZE
				/ BYTE_PER_MESSAGE;

		private static final int ENTRY_PER_SEGMENT = SEGMENT_SIZE
				/ BYTE_PER_ENTRY;
		...
		public long read(MessageId id) throws IOException {
			int position = m_header.getOffset(id.getIpAddressValue(),
					id.getIndex(), false);
		...
		public void write(MessageId id, long blockAddress, int blockOffset)
				throws IOException {
			int position = m_header.getOffset(id.getIpAddressValue(),
					id.getIndex(), true);
	...
	private Metric m_indexMetric = new Metric("index");

	private Metric m_dataMetric = new Metric("data");
	...
	private void ensureOpen(MessageId id) throws IOException {
		if (!m_index.isOpen()) {
			...
			File dataPath = m_bulider.getFile(domain, startTime, ip,
					FileType.DATA);
			File indexPath = m_bulider.getFile(domain, startTime, ip,
					FileType.INDEX);
	...
	@Override
	public void setBenchmark(Benchmark stopWatch) {
		if (m_indexMetric == null) {
			m_indexMetric = stopWatch.get("index");
			m_dataMetric = stopWatch.get("data");
		}
	}
