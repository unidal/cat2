...
@Named(type = MessageTreeProducer.class)
public class DefaultMessageTreeProducer implements MessageTreeProducer {
   @Inject
   ...
   private boolean shouldLog(Throwable e) {
      if (m_manager instanceof DefaultMessageTreeManager) {
         return ((DefaultMessageTreeManager) m_manager).shouldLog(e);
      } else {
         return true;
      }
   }
