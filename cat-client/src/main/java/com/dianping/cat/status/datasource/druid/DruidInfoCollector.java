...
public class DruidInfoCollector extends DataSourceCollector {
	...
	private Map<String, Number> doCollect() {
		...
		for (Map.Entry<String, DruidMonitorInfo> entry : druidMonitorInfoMap.entrySet()) {
			...
			map.put(PREFIX_KEY + SPLIT + dataSourceName + ".idle_connection",
			      value.getPoolingCount() - value.getActiveCount());
			...
			if ("true".equals(detail)) {
				...
				map.put(PREFIX_KEY + SPLIT + dataSourceName + ".close_prepared_statement_count",
				      value.getClosedPreparedStatementCount());
				...
				map.put(PREFIX_KEY + SPLIT + dataSourceName + ".avg_create_timespan_millis",
				      value.getAvgCreateTimespanMillis());
				...
				map.put(PREFIX_KEY + SPLIT + dataSourceName + ".max_open_prepared_statements",
				      value.getMaxOpenPreparedStatements());
				map.put(PREFIX_KEY + SPLIT + dataSourceName + ".max_pool_prepared_statement_pre_connection_size", entry
				      .getValue().getMaxPoolPreparedStatementPerConnectionSize());
				...
				map.put(PREFIX_KEY + SPLIT + dataSourceName + ".avg_not_empty_wait_millis",
				      value.getAvgNotEmptyWaitMillis());
				map.put(PREFIX_KEY + SPLIT + dataSourceName + ".not_empty_wait_thread_count",
				      value.getNotEmptyWaitThreadCount());
	...
	private DruidMonitorInfo getDruidMonitorInfo(ObjectName objectName) {
		...
		druidMonitorInfo
		      .setMaxOpenPreparedStatements(getIntegerAttribute(objectName, "MaxOpenPreparedStatements", false));
		druidMonitorInfo.setMaxPoolPreparedStatementPerConnectionSize(getIntegerAttribute(objectName,
		      "MaxPoolPreparedStatementPerConnectionSize", false));
