...
import org.unidal.cat.plugin.events.report.view.GraphPayload.FailurePayload;
import org.unidal.cat.plugin.events.report.view.GraphPayload.HitPayload;
...
public class GraphViewModel {
   ...
   public GraphViewModel(GraphBuilder builder, String bu, String type, String name, EventsReport report) {
   ...
   public GraphViewModel(String bu, String type, String name, EventsReport current, EventsReport last,
         EventsReport baseline) {
   ...
   private double[] aggregateMetrics(EventsReport report, String ip, String type, String name, String metric, int size) {
   ...
   private void buildBarGraphs(GraphBuilder builder, EventsReport report, String bu, String type, String name) {
      EventsType t = report.findOrCreateDepartment(bu == null ? Constants.ALL : bu).findOrCreateType(type);
      EventsName n = t.findOrCreateName(name == null ? Constants.ALL : name);
   ...
   private void buildDistribution(EventsReport report, String type, String name) {
   ...
   private LineChart buildLineChart(String ip, String type, String name, EventsReport current, EventsReport last,
         EventsReport baseline, String metric) {
   ...
   private void buildLineCharts(String ip, String type, String name, EventsReport current, EventsReport last,
         EventsReport baseline) {
   ...
   private void buildPieChart(EventsReport report, String type, String name) {
      ...
      public void visitName(EventsName name) {
         if (m_name.equals(name.getId())) {
            ...
            detail.setTotalCount(name.getTotalCount()).setFailCount(name.getFailCount())
                  .setFailPercent(name.getFailPercent()).setIp(m_bu);
         ...
         if (m_type != null && m_type.equals(type.getId())) {
            if (StringUtils.isEmpty(m_name)) {
               ...
               detail.setTotalCount(type.getTotalCount()).setFailCount(type.getFailCount())
                     .setFailPercent(type.getFailPercent()).setIp(m_bu);
               m_details.add(detail);
            } else {
   ...
   private static class MetricsAggregator extends BaseVisitor {
      ...
      @Override
      public void visitRange(EventsRange range) {
