...
public class EventsReportReducerTest extends ComponentTestCase {
   ...
   public static class MockReportStorage implements ReportStorage<EventsReport> {
      @Override
      public List<EventsReport> loadAll(ReportDelegate<EventsReport> delegate, ReportPeriod period,
            Date startTime, String domain) throws IOException {
         List<EventsReport> reports = new ArrayList<EventsReport>();

         for (int i = 1; i <= 3; i++) {
            EventsReport report = loadReport(delegate, period.getName() + "-" + i + ".xml");
      ...
      @Override
      public List<EventsReport> loadAllByDateRange(ReportDelegate<EventsReport> delegate,
      ...
      private EventsReport loadReport(ReportDelegate<EventsReport> delegate, String resource)
            throws IOException {
      ...
      @Override
      public void store(ReportDelegate<EventsReport> delegate, ReportPeriod period, EventsReport report,
            int index) throws IOException {
         EventsReport expected = loadReport(delegate, period.getName() + ".xml");

         Assert.assertEquals(String.format("EventsReport(%s) mismatched!", period), expected.toString(),
               report.toString());
      }
   }
