...
public class NameViewModel implements TableViewModel<NameEntry> {
	...
	public static class NameEntry {
		...
		public NameEntry(TransactionName name) {
			m_name = name;
		}

		public NameEntry(TransactionName name, boolean summary) {
			m_name = name;
			m_summary = summary;
		}
	...
	private class NameHarvester extends BaseVisitor {
		private Map<String, List<TransactionName>> m_map = new HashMap<String, List<TransactionName>>();
		...
		public void harvest(List<NameEntry> names) {
			...
			for (Map.Entry<String, List<TransactionName>> e : m_map.entrySet()) {
				List<TransactionName> list = e.getValue();
				TransactionName name;

				if (list.size() == 1) {
					name = list.get(0);
				} else {
					name = new TransactionName(e.getKey());

					for (TransactionName item : list) {
		...
		public void visitName(TransactionName name) {
			...
			List<TransactionName> list = m_map.get(id);

			if (list == null) {
				list = new ArrayList<TransactionName>();
				m_map.put(id, list);
			}
	...
	private static class QueryFilter {
		...
		public boolean apply(TransactionName name) {
